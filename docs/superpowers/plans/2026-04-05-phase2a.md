...
**Goal:** Add PR creation/viewing, branch stacking, branch import, and multi-repo task display to hive.

**Architecture:** New `src/pr.ts` module for PR operations via `gh` CLI. Existing modules modified: `types.ts` (data model), `tasks.ts` (stacking/import), `tui.ts` (key handlers, rendering), `background.ts` (PR cache refresh).
...
Run: `~/.deno/bin/deno check cli.ts`
Expected: no errors
...
Run: `~/.deno/bin/deno test src/pr_test.ts --allow-env --allow-read --allow-write`
...
Run: `~/.deno/bin/deno test --allow-env --allow-read --allow-write --allow-run`
Expected: all PASS
...
```typescript
import { createPr, openPrInBrowser, getPrInfo } from "./pr.ts";
