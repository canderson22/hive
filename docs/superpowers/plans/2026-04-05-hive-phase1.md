...
**Goal:** Build the core loop of hive — a terminal dashboard that manages multiple parallel Claude Code sessions in tmux with git worktree isolation and hook-based status monitoring.

**Architecture:** Native Deno TypeScript. Shells out to `git` and `tmux` via `Deno.Command`. Signal files written by Claude Code hooks drive a polling monitor. Custom ANSI rendering for the live dashboard, `@clack/prompts` for dialog flows. Config and state persisted as JSON in `~/.hive/`.
...
Run: `deno test src/paths_test.ts`
Expected: PASS (types and paths are already implemented above — this validates the code)
...
Run: `deno run --allow-env cli.ts --version`
Expected: `hive 0.1.0`
...
Run: `deno test src/config_test.ts --allow-env --allow-read --allow-write`
...
Run: `deno test src/git_test.ts --allow-env --allow-read --allow-write --allow-run`
Expected: FAIL — module `./git.ts` not found
...
Run: `deno test src/git_test.ts --allow-env --allow-read --allow-write --allow-run`
Expected: FAIL — `hasReadyWorktree`, `ensureReadyWorktree`, `consumeReadyWorktree` not exported
...
Run: `deno test src/hooks_test.ts --allow-env --allow-read --allow-write --allow-run`
Expected: FAIL — module `./hooks.ts` not found
...
Run: `deno test src/monitor_test.ts --allow-env --allow-read --allow-write`
Expected: FAIL — module `./monitor.ts` not found
...
Run: `deno check src/keypress.ts`
Expected: no errors
...
Run: `deno test src/ansi_test.ts --allow-env --allow-read`
Expected: FAIL — module `./ansi.ts` not found
...
This module orchestrates git, tmux, hooks, and state. It's tested via integration (requires git and tmux) rather than pure unit tests.
...
import { hiveHome, repoPath, worktreePath, readyWorktreePath, repoNameFromUrl } from "./paths.ts";
import {
  ensureBareClone,
  createWorktree,
  removeWorktree,
...
import { createSession, killSession, hasSession, sendKeys } from "./tmux.ts";
import { installSignalScript, installHooksConfig } from "./hooks.ts";
...
Run: `deno test src/tui_test.ts --allow-env --allow-read`
Expected: FAIL — module `./tui.ts` not found
...
import { createTask, restartTask, closeTask, openEditor, type CreateTaskOpts } from "./tasks.ts";
import { loadConfig, saveConfig, loadState, saveState } from "./config.ts";
import { enableRawMode, disableRawMode, readKey } from "./keypress.ts";
import {
  bold,
  dim,
  green,
...
Run: `deno compile --allow-env --allow-read --allow-write --allow-run --allow-net --output=/tmp/hive-test cli.ts`
Expected: binary created at `/tmp/hive-test`

Run: `/tmp/hive-test --version`
Expected: `hive 0.1.0`
